...
class ChatWidget(QWidget):
    ...
    def __init__(self, current_user, contact, parent=None):
        ...
        print(f"Создание чата с {contact.name}...")
        self.init_ui()
        self.load_messages()
    ...
    def load_messages(self):
        try:
            ...
            # ✅ ПРАВИЛЬНО: messages_data (полное имя)
            ...
            print(f"Ошибка load_messages: {e}")
    ...
    def send_message(self):
        try:
            ...
            msg_id = send_message(self.current_user.uid, self.contact.uid, text)
            
            if msg_id:
                from datetime import datetime
                msg = Message(id=msg_id, from_uid=self.current_user.uid, to_uid=self.contact.uid, text=text, timestamp=datetime.now(), delivered=True)
    ...
    def listen_for_messages(self):
        try:
            ...
            self.message_listener = listen_for_messages(user_uid=self.current_user.uid, callback=on_new_message)
        except Exception as e:
